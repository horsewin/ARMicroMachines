...
//	osg::ref_ptr<osg::Node> car1 = osgDB::readNodeFile("Data/Cars/HatuneMiku.3ds");
	//osg::ref_ptr<osg::Node> car1 = osgDB::readNodeFile("Data/Cars/Voxel.3ds");
  ...
  car_trans1->setPosition(osg::Vec3d(0.0, 0.0, 3.0));//shift body higher 3 units
	//car_trans1->setScale(osg::Vec3d(0.5, 0.5, 0.5));
	//car_trans1->setScale(osg::Vec3d(500,500,500));
...
////this function have not been used anymore(Dec 14, 2011)
//void osg_createHand(int index, float x, float y, float world_scale) 
//{
//	float sphere_size = 0.7;
//	//float sphere_size = world_scale;
...
//			shape->setColor(osg::Vec4d(1,1,1,1));
//			osg::ref_ptr< osg::Geode> geode = new osg::Geode();
...
//			hand_object_transform_array[index].at(curr)->setPosition(osg::Vec3d(j*SPHERE_SCALE, i*SPHERE_SCALE, SPHERE_SCALE));
	...
	for(int i = 0; i < MIN_HAND_PIX; i++) {
		for(int j = 0; j < MIN_HAND_PIX; j++) {
			...
			if(grid[curr] > 0)
				...
				hand_object_transform_array[index].at(curr)->setPosition(osg::Vec3d(x[curr]*SPHERE_SCALE, y[curr]*SPHERE_SCALE, 1000*DEPTH_SCALE));
